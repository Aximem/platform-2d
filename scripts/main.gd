...
func _ready() -> void:
	...
	platform_initial_position = tile_map_layer_moving_platform.position

func _process(delta: float) -> void:
	_move_platform(delta)

func _move_platform(delta: float) -> void:
	tile_map_layer_moving_platform.position.x += platform_speed * platform_direction * delta

	var current_offset = tile_map_layer_moving_platform.position.x - platform_initial_position.x

	# Atteint la limite droite (500px)
	if current_offset >= platform_max_distance:
		tile_map_layer_moving_platform.position.x = platform_initial_position.x + platform_max_distance
		platform_direction = -1
	# Revenu à la position initiale
	elif current_offset <= 0:
		tile_map_layer_moving_platform.position.x = platform_initial_position.x
		platform_direction = 1
